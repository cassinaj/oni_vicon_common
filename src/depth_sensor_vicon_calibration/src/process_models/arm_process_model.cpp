...
ArmProcessModel::ArmProcessModel(float angle_sigma, float trans_sigma, Eigen::Vector3f rot_center)
...
ArmProcessModel::~ArmProcessModel(){}

vector<float> ArmProcessModel::Sample(vector<float> initial_state, double delta_time, std::vector<float> controls)
{
//	if(controls.size() != 7)
//	{
//		cout << "error: you have go give the quaternion and the translation of the hand holding the object " << endl;
//		exit(-1);
//	}
	Quaternionf q_control = Quaternionf(controls[0], controls[1], controls[2], controls[3]);
	Vector3f t_control = Vector3f(controls[4], controls[5], controls[6]);
	...
	// we simply apply the control input to the initial state, and then we add some noise ---------------------------------------
	q_init = q_control.toRotationMatrix()*q_init.toRotationMatrix();
	t_init = q_control.toRotationMatrix()*t_init + t_control;
