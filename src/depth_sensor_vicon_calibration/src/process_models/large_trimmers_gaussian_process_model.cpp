#include "large_trimmers_gaussian_process_model.hpp"
#include <Eigen/Dense>
#include <iostream>
...
LargeTrimmersGaussianProcessModel::LargeTrimmersGaussianProcessModel
...
vector<float> LargeTrimmersGaussianProcessModel::Sample(vector<float> initial_state, double delta_time, std::vector<float> controls)
{
	...
	final_state[7] = initial_state[7] + opening_generator_()*delta_time;

	if(final_state[7] < min_opening)
		final_state[7] = min_opening + (min_opening - final_state[7]);
	if(final_state[7] > max_opening)
		final_state[7] = max_opening - (final_state[7] - max_opening);
